...
public:
    QpProblem(s64 n_, s64 m_, bool print_timings = true) :
        A(m_, n_), P(n_, n_), l(m_), u(m_), q(n_), n(n_), m(m_), _print(print_timings), _kkt(n_ + m_, n_ + m_), _cholDenseSolver(print_timings),
        _xzTilde(n_ + m_), _y(m_), _x0(n_), _x1(n_), _z0(m_), _z1(m_), _Ar(m_), _Pr(n_), _AtR(n_), _deltaY(m_), _AtDeltaY(n_), _deltaX(n_), _PDeltaX(n_),
        _ADeltaX(m_)
    {
        q.setZero();
