...
// scale to -1 ~ 1
static float scale_joystick(uint16_t in) { return (in - 172) * 2.f / (1811.f - 172.f) - 1.f; }
...
void update_taranis_at9s(AT9s_data* data)
{
    ...
    data->SWB  = map_switch_bool(channel_data[5]); // 第六通道为swB    // changed by EHL, 4DAGE, Zhuhai
    ...
    /*   static int count_tims(0);
       count_tims++;
       if(count_tims%10){
           printf("right x: %.2f\t%.2f\t%.2f\t%.2f\t%d\t%d \t%d\t%d\t%d\t%d\t%d\n",
                  data->right_stick_x,
                  data->right_stick_y,
                  data->left_stick_x,
                  data->left_stick_y,
                  data->SWE,
                  channel_data[4],
                  channel_data[5],channel_data[6],channel_data[7],channel_data[8],channel_data[9]);
       }*/
}
